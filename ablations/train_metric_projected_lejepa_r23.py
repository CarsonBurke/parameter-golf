"""
LeJEPA / LeWM analog r23: cosine (direction-only) regression variant of r22.

r22 plateaued with attract ~0.80 early (step ~200) and stayed flat despite
val_bpb still improving. Post-mortem: MSE on raw (unnormalized) target embeds
has a floor set by (a) the per-dim variance of tok_emb (~1.0, pinned by
sigreg_tok) and (b) growing row-norm heterogeneity (tok_row_norm_std grew
1.47 -> 2.28 by step 560). Frequent tokens accumulate larger row norms since
SIGReg regularizes the flat cross-token distribution but not per-row norms,
so for any frequent high-norm row the MSE floor is ~(norm^2 / D) > 1. MSE
also penalizes "close-but-wrong-direction" equivalently to "far-wrong" which
is a poor surrogate for tied-embedding CE (which is dominated by direction).

r23: replace MSE-on-raw-embeds with cosine regression. Both flat_h and the
stop-grad target_embed are unit-normalized, then attract = 1 - cos(flat_h,
target). This:
- Removes row-norm heterogeneity from the attract objective (direction only)
- Matches the geometry the eval softmax cares about most — dot products are
  dominated by direction once norms are comparable
- Keeps the LeJEPA recipe: stop-grad target, sigreg_tok on the view set

sigreg_tok still lives on tok_emb.weight (unchanged). It caps the full-set
isotropy so tok_emb stays near-orthogonal and unit-variance per dim; that
keeps cosine meaningful (near-random pairs have cos ~ 0, attract bound ~1).

Loss:
  flat_h_unit = F.normalize(flat_h, dim=-1)
  target_unit = F.normalize(stopgrad(tok_emb[target]), dim=-1)
  attract     = (1 - (flat_h_unit * target_unit).sum(dim=-1)).mean()
  sigreg_tok  = Epps-Pulley across tok_emb rows (view-set isotropy)
  total       = attract + sigreg_weight * sigreg_tok

tok_emb gradients: (1) input-side from the trunk, (2) sigreg_tok. No collapse
pathway through attract since target_embed is detached.

Eval: unchanged — standard tied-embedding vocab CE / BPB.
"""
...
class LeJEPAMetricGPT(base.GPT):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Teacher is stop-grad: attract only shapes the predictor (flat_h),
        # never the embedding table. tok_emb still updates via the input-side
        # pathway of the trunk, just like a normal LM — we only kill the
        # target-side gradient so attract can't inflate row norms of frequent
        # tokens.
        target_embed = self.tok_emb(targets).detach()
        # Cosine regression: both sides unit-normalized, so attract measures
        # direction mismatch only. Removes the MSE floor induced by tok_emb
        # row-norm heterogeneity (frequent tokens drifting to larger norms)
        # that plateaued r22 at ~0.80. Bounded in [0, 2], ~1 for random pairs.
        flat_h_unit = F.normalize(flat_h.float(), dim=-1)
        target_unit = F.normalize(target_embed.float(), dim=-1)
        attract = (1.0 - (flat_h_unit * target_unit).sum(dim=-1)).mean()
