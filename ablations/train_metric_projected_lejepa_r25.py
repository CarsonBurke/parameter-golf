"""
LeJEPA / LeWM analog r25: r24 with SIGReg removed from the loss.

r24 observation: InfoNCE attract + SIGReg on tok_emb actively diverged —
attract rose from 6.78 → 7.66 over 300 steps while val_bpb degraded
4.01 → 4.47 and tok_row_norm_std climbed despite sigreg_weight=10. Diagnosis:
SIGReg pushes tok_emb toward isotropic Gaussian, which is adversarial to the
semantic clustering InfoNCE needs to work. The two losses fight and the
predictor loses.

Hypothesis here: with the target side detached (attract does not gradient
into tok_emb), there is no collapse pathway — tok_emb's only gradient source
becomes the input-side trunk, which already shapes embeddings toward useful
LM structure via the rest of the network. Row norms may stabilize on their
own without SIGReg's pressure toward isotropy.

Change vs r24: drop sigreg_tok from the training loss. Keep the SIGReg
computation and all manifold diagnostics so we can watch row-norm,
eff-rank, and sigreg_tok evolve organically — just don't add them to loss.

Loss:
  attract = CE(softcap · tanh(flat_h @ stopgrad(tok_emb).T / softcap), target)
  total   = attract       (no SIGReg term)

tok_emb gradient sources: (1) input-side trunk only. No target-side grad
from attract (detached), no SIGReg pressure.
...
class LeJEPAMetricGPT(base.GPT):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # SIGReg computed for diagnostics only — not added to the loss.
        sigreg_tok = self.sigreg_loss(self.tok_emb.weight).detach()

        self.last_attract.copy_(attract.detach())
        self.last_sigreg_tok.copy_(sigreg_tok)
        return attract
