"""
LeJEPA / LeWM analog r21: predictor regresses to next-token embedding, SIGReg
on the predictor output keeps it from collapsing.

Setup mirrors the paper: trunk outputs `flat_h` (the predictor), target is
`tok_emb[next_token]` (the teacher-side lookup). Attract is MSE regression,
teacher is **stop-gradient** — attract only shapes the predictor, never the
embedding table. Anti-collapse is a single SIGReg term on the predictor's
output distribution (Epps-Pulley on flat_h), which is exactly the paper's
default (λ*SIGReg + regression, λ small).

Why stop-grad on the teacher is the crux:
- Earlier variants (r14-r20) let attract backprop into tok_emb, which created
  the well-known collapse pathology in non-contrastive SSL: predictor and
  teacher race toward a shared low-rank space. We piled SIGReg on tok_emb,
  cross-similarity SIGReg, etc. to fight it — none won against Adam + attract
  on frequent-token rows. With stop-grad the race disappears: tok_emb only
  moves via the trunk's *input* embedding pathway (normal LM signal), never
  via the attract pathway.
- tok_emb now plays the role of a frozen target encoder (like LeJEPA's EMA
  teacher). It isn't static (input-side gradient still updates it) but
  attract no longer inflates its row norms.
- Since only flat_h is a "view" we're regularizing, SIGReg lives only on
  flat_h. No sigreg_tok, no cross-sigreg.

Loss:
  attract = MSE(flat_h, stopgrad(tok_emb[target]))
  sigreg  = Epps-Pulley on flat_h rows
...
class LeJEPAMetricGPT(base.GPT):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        return attract + self.sigreg_weight * sigreg_pred
