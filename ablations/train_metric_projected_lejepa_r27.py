"""
LeJEPA / LeWM analog r27: r22 with the target-side stop-grad removed —
faithful translation of LeJEPA (Balestriero & LeCun, arXiv:2511.08544) to LM.

The LeJEPA paper's "no stop-gradient" is load-bearing, not marketing:
§5.1 states "The absence of prototypes, stop-gradients, and teacher-student
networks makes LeJEPA appealing." §6.1 explains that predictors and
teacher-student machinery exist in prior JEPAs only to prevent collapse,
and that SIGReg provably substitutes for their role. The paper's
prediction loss (eq 7) is plain L2 between live view embeddings:
  L_pred = (1/V) Σ_v ‖ μ_n − z_{n,v} ‖²,   μ_n = mean of global views
with gradient flowing through both sides. No .detach() anywhere.

Our r22 inherited a .detach() on tok_emb(targets) from earlier JEPA-family
LM adaptations (r10–r21 lineage). That detach is a departure from LeJEPA
that it neither tests nor endorses. r27 removes it: MSE attract on
tok_emb(targets) with gradient flowing both ways, plus SIGReg on
tok_emb.weight. This is literal LeJEPA mapped onto token-level prediction.

Gradient topology for tok_emb.weight in r27:
  (1) input-side trunk (unchanged)
  (2) attract via target lookup — gradient now flows (removed detach)
  (3) sigreg_tok — unchanged
Collapse is prevented by SIGReg (per LeJEPA's theorems), not by stop-grad.
...
class LeJEPAMetricGPT(base.GPT):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # No stop-grad on the target: per LeJEPA, SIGReg on tok_emb.weight
        # substitutes for the collapse-prevention role of stop-grad. Attract
        # gradient flows into tok_emb via the target lookup as well as into
        # flat_h, matching LeJEPA's between-live-views L2 objective.
