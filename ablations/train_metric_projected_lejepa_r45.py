"""
LeJEPA r45: cosine-InfoNCE over a 2-prototype token manifold.
...
r45 keeps r44's relative state temperature and weak pred-side SIGReg, but
changes the output object from "one token = one point" to "one token = a
tiny manifold":

  hidden      = encode_hidden(x)                              # [B, T, D]
  z           = F.normalize(hidden, dim=-1)                   # unit-sphere
  base(v)     = F.normalize(tok_emb[v], dim=-1)               # input/output anchor
  extra(v)    = F.normalize(tok_emb[v] + Δv, dim=-1)          # learned manifold point
  temp_delta  = tanh(w_temp·z_i + b_temp) * Δ                   # bounded
  temp_delta  = temp_delta - mean(temp_delta)                   # relative-only
  log_temp_i  = clamp(log_temp + temp_delta, [min, max])
  s_base      = exp(log_temp_i) * cos(z, base(v))
  s_extra     = exp(log_temp_i) * cos(z, extra(v))
  logits_v    = logaddexp(s_base, s_extra)                    # token manifold score
...
What changes vs r44:
- Each token now has two output prototypes: the base token row and one learned
  residual prototype. The score for a token is the log-sum-exp across those
  two points, so the predictor can land in a small region of embedding space
  instead of matching a single direction exactly.
- The extra prototype is output-only. Input embeddings stay tied to `tok_emb`;
  the manifold branch only changes retrieval geometry.
- The extra prototype starts exactly on top of the base row (`Δv = 0`), so the
  initial decoder is mathematically equivalent to r44 up to an additive `log 2`
  constant that cancels in the softmax. Any gain must come from the learned
  manifold, not from a warm-start trick.
- State temperature and weak pred-side SIGReg stay unchanged from r44.
...
Risks vs r44:
- The extra prototype can stay collapsed onto the base row, making the branch
  a no-op.
- Frequent tokens could use the extra prototype as a frequency-specific bias
  bucket rather than a semantic manifold.
- The state-dependent temperature can still become a confidence shortcut. If
  that happens, temp_std will widen and the codebook geometry will clump even
  if train CE keeps falling.
...
Hyperparameters (env):
  ...
  LEJEPA_PRED_SIGREG_WEIGHT — additive weight on predictor-side SIGReg,
                              default 0.01.
  LEJEPA_PROTO_LR        — Adam LR for the extra manifold prototype table,
                           default 0.01.

Diagnostics: sigreg_tgt, sigreg_pred, sigreg_h (read-only), temp_mean,
temp_std, target-side base/extra prototype gap surfaced each step; tok_emb
and extra-prototype manifold diagnostics every val_loss_every.
...
class Hyperparameters(base.Hyperparameters):
    ...
    proto_lr = float(os.environ.get("LEJEPA_PROTO_LR", "0.01"))


class LeJEPAManifoldGPT(base.GPT):
    def __init__(self, args: Hyperparameters):
        ...
        # Global base log-temperature plus a tiny bounded relative per-state
        # offset. The head only makes local adjustments; the learned scalar
        # log_temp remains the sole global calibration channel.
        ...
        # Output-only residual prototype: each token gets one extra point in
        # embedding space, initialized exactly at the base token row.
        self.proto_resid = nn.Parameter(torch.zeros(args.vocab_size, args.model_dim))
        ...
        self.register_buffer("last_proto_gap", torch.zeros((), dtype=torch.float32), persistent=False)
    ...
    def manifold_diagnostics(self) -> dict[str, float]:
        ...
        extra = tok + self.proto_resid.float()
        extra_unit = F.normalize(extra, dim=-1)
        ...
        return {
            ...
            "proto_resid_norm_mean": float(self.proto_resid.float().norm(dim=-1).mean().item()),
            "proto_resid_norm_std": float(self.proto_resid.float().norm(dim=-1).std(correction=0).item()),
            "proto_cos_base_extra_mean": float((tok_unit * extra_unit).sum(dim=-1).mean().item()),
            "sigreg_proto_extra": float(self.sigreg_loss(extra).item()),
    ...
    def cosine_logits(self, flat_hidden: Tensor) -> tuple[Tensor, Tensor]:
        ...
        base_codebook = F.normalize(self.tok_emb.weight.float(), dim=-1)
        extra_codebook = F.normalize(self.tok_emb.weight.float() + self.proto_resid.float(), dim=-1)
        ...
        base_logits = (z @ base_codebook.T) * temp.unsqueeze(-1)
        extra_logits = (z @ extra_codebook.T) * temp.unsqueeze(-1)
        return torch.logaddexp(base_logits, extra_logits), temp
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        with torch.no_grad():
            ...
            target_base = F.normalize(target_emb.float(), dim=-1)
            target_extra = F.normalize(target_emb.float() + self.proto_resid[tgt_flat].float(), dim=-1)
            proto_gap = (1.0 - (target_base * target_extra).sum(dim=-1)).mean()
        ...
        self.last_proto_gap.copy_(proto_gap.detach())
...
def main() -> None:
    ...
    base_model = LeJEPAManifoldGPT(args).to(device).bfloat16()
    ...
    matrix_params = [
        ...
        if name not in {"tok_emb.weight", "lm_head.weight", "proto_resid"}
        and p.ndim == 2
    ...
    optimizer_proto = torch.optim.Adam(
        [{"params": [base_model.proto_resid], "lr": proto_lr, "base_lr": proto_lr}],
        betas=(args.beta1, args.beta2),
        eps=args.adam_eps,
        fused=True,
    )
    ...
    optimizers: list[torch.optim.Optimizer] = [optimizer_tok, optimizer_proto, optimizer_muon, optimizer_scalar]
    ...
    log0(
        ...
        f"proto_lr={proto_lr} "
        f"head=cosine_infonce_dual_proto_reltemp"
    )
    log0(
        f"tie_embeddings:{args.tie_embeddings} embed_lr:{token_lr} proto_lr:{proto_lr} "
    ...
    while True:
        ...
        if step % 10 == 0 or last_step:
            log0(
                ...
                f"proto_gap:{base_model.last_proto_gap.item():.4f} "
        ...
        if step % args.val_loss_every == 0 or last_step:
            ...
            log0(
                ...
                f"proto_resid_norm_mean:{diag['proto_resid_norm_mean']:.4f} "
                f"proto_resid_norm_std:{diag['proto_resid_norm_std']:.4f} "
                f"proto_cos_base_extra_mean:{diag['proto_cos_base_extra_mean']:.4f} "
                f"sigreg_proto_extra:{diag['sigreg_proto_extra']:.4f} "
