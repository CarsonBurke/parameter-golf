"""
LeJEPA / LeWM analog r22: predictor regresses to next-token embedding, SIGReg
lives on the *token embedding distribution* (across tokens), not on the
predictor.

r21 (SIGReg on flat_h) plateaued at bpb ~3.65 because SIGReg pinned flat_h's
per-dim std to 1, while tok_emb drifted larger (per-dim std ~1.57 by step
1100) from input-side gradients. attract = MSE(flat_h, tok_emb[target]) then
had a scale-mismatch lower bound it couldn't cross.

Fix here: move SIGReg to tok_emb.weight. sigreg_loss over a [V, D] tensor
reduces with mean(dim=0) — it tests the marginal distribution of V=32000 rows
projected onto P random directions against N(0, I). That's "isotropy across
views" in LeJEPA's framing — each token row is a view, and SIGReg says the
view set is marginally Gaussian (near-orthogonal + unit-scale in high D).

This:
- Caps tok_emb's scale drift (root cause of r21's plateau)
- Keeps tok_emb full-rank and orthogonal-ish across tokens
- Leaves flat_h free to follow attract without scale lock-in; flat_h
  inherits its distribution from the target side via regression
...
Attract is still stop-grad on target. tok_emb's only gradient paths are:
(1) input-side from the trunk, (2) sigreg_tok. No collapse pathway.
