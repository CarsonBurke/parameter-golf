"""
LeJEPA / LeWM analog r29: direct LeJEPA analog for LM.
  attract = ‖ flat_h − tok_emb(targets) ‖²   (no .detach() anywhere)
  sigreg  = Epps-Pulley on flat_h (the trunk's encoder-output samples)

Why flat_h for SIGReg: LeJEPA applies SIGReg to encoder outputs z_{n,v} =
f_θ(x_{n,v}). For LM, the encoder is the trunk (encode_hidden); its output
is flat_h (one vector per token position per sequence). Analogous.

Prior attempts and why r29 should beat them:
- r21: SIGReg on flat_h but with .detach() on target. Scale mismatch —
  SIGReg pinned flat_h's std=1 while tok_emb drifted from input-side
  gradients only (detach blocked attract from shaping it). In r29, no
  detach → attract pulls tok_emb into the SIGReg-shaped region, no gap.
- r22: SIGReg on tok_emb.weight with detach. Works, but is two departures
  from LeJEPA.
- r27: no detach but SIGReg on tok_emb.weight (parameter matrix). Attract
  is frequency-weighted per-row; SIGReg treats rows uniformly → row-norm
  heterogeneity and partial collapse.
- r28: SIGReg on tok_emb(input_ids) (realized samples). Delta-mass
  pathology from deterministic lookup → scale collapse.

r29 anchors SIGReg on the same distribution whose samples attract operates
on (flat_h), matching LeJEPA's pattern of co-locating attract and SIGReg
on encoder outputs.

Known risk (from red-team): N(0,I) isotropy may be anti-aligned with CE-
calibrated logit geometry, which prefers frequency-aligned anisotropy
(frequent tokens with larger norms to dominate softmax). If r29 trains
stably but plateaus on val_bpb worse than r22, this is the reason, and the
follow-up is to put SIGReg on a projected flat_h to decouple isotropy
from the main representation.
...
class LeJEPAMetricGPT(base.GPT):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # MSE attract with no stop-grad — gradient flows into both sides,
        # per LeJEPA. Attract pulls flat_h and tok_emb[target] together.
        target_embed = self.tok_emb(targets)
        attract = (flat_h - target_embed).pow(2).mean()

        # SIGReg on flat_h: the trunk output is the analog of LeJEPA's
        # encoder output z_{n,v}. Co-located with attract on the same sample
        # distribution. tok_emb.weight has no direct regularizer — its rows
        # are shaped only via (a) input-side trunk grad, (b) attract pulling
        # row_t toward the flat_h cluster for "contexts predicting t".
        sigreg_tok = self.sigreg_loss(flat_h)
