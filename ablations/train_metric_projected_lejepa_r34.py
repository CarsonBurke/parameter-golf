"""
LeJEPA r34: depth-matched deep-target JEPA for LM.
  hidden    = encode_hidden(x)             # [B, T, D], post-final-norm
  h_ctx     = hidden[:, :-1, :]            # position-t hidden (context)
  h_tgt     = hidden[:, 1:, :].detach()    # position-(t+1) hidden, stop-grad
  attract   = ‖ h_ctx − h_tgt ‖²           # L_pred, predictor = identity
  sigreg_h  = Epps-Pulley on flat_h        # on encoder-output distribution
  loss      = (1-λ)·attract + λ·sigreg_h

Why this is more structurally faithful than r22–r30:
- r22–r30 all compared a DEEP h_t (trunk output) to a SHALLOW target
  (tok_emb lookup row, depth-1). LeJEPA compares z_{n,v} to z_{n,v'}, both
  of which are outputs of the SAME encoder applied to different views —
  depth-matched. r34 fixes this by using h_{t+1} = encoder(x_{1:t+1}) as
  the target: same encoder, same depth, one position shifted.
- Target is now a continuous encoder output (Sobolev α > 0), so SIGReg's
  consistency theory (Thm 5, B.9) applies. No atomic-measure pathology.
- Stop-grad on the target side: NEPA (arXiv 2512.16922) empirically
  showed that even with continuous patch targets, removing stop-grad in
  autoregressive setups collapses to a constant. We keep sg on h_tgt to
  break the collapse-together dynamic. LeJEPA's paper avoids this via
  symmetric V-view averaging (μ_n), which our single-forward causal setup
  can't replicate.

Known risk: eval BPB.
  tok_emb.weight receives gradient only via the INPUT-side lookup (as
  usual). Attract does NOT pull h_t toward tok_emb(target_t+1), so the
  tied-head decoder direction is not directly trained. Eval CE
  (logits = hidden @ tok_emb.weight.T) may be bad even if attract/sigreg
  train a healthy trunk. This is expected — a proper LeJEPA port would
  use a learned linear probe for eval. We keep tied-head CE for BPB to
  match the benchmark; if val_bpb is garbage but sigreg_h/attract look
  healthy, we've confirmed the training dynamics work and eval alignment
  is the separate, orthogonal problem.

Prior attempts:
- r22 (MSE + sg + SIGReg on tok_emb.weight): depth-mismatched, but tied
  head decodes. val_bpb 3.53 @ 1000.
- r27–r30: kept depth-mismatch, tried to fix SIGReg placement; all
  collapsed because atomic-target SIGReg has degenerate minima (Cramér-
  Wold test misspecified for singular P vs continuous Q).
- r34: depth-matched, no atomic target, SIGReg's theory applies. Tests
  whether the LeJEPA training dynamics transfer to LM at all, decoupled
  from tied-head eval alignment.

Diagnostics:
- sigreg_tok (diag): SIGReg on tok_emb.weight. Reference — NOT backprop'd.
  Trained only by input-side gradient, so tells us whether tok_emb drifts
  into a reasonable geometry without explicit pressure.
- sigreg_h_fwd: per-step sigreg_h (backprop'd).
- attract: per-step MSE(h_ctx, h_tgt).

Eval: tied-head CE for BPB, as usual. See "Known risk" above.
"""
...
class LeJEPAMetricGPT(base.GPT):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Depth-matched JEPA: both "views" are encoder outputs of the same
        # trunk, offset by one position. h_ctx is the position-t hidden
        # that sees x_{1:t}; h_tgt is the position-(t+1) hidden that sees
        # x_{1:t+1}. Causal mask guarantees h_ctx doesn't see x_{t+1}, so
        # the attract task is non-trivial. Stop-grad on h_tgt prevents
        # collapse-together through the shared trunk (NEPA 2512.16922).
        h_ctx = hidden[:, :-1, :]
        h_tgt = hidden[:, 1:, :].detach()
        attract = (h_ctx - h_tgt).pow(2).mean()

        # SIGReg on the full encoder-output distribution (all positions).
        # This is the "SIGReg per view" analog — we have a single forward,
        # so a single sigreg term covers both the context and target sides
        # (they draw from the same post-final-norm hidden distribution).
        ...
        if self.use_additive:
            return attract + self.sigreg_weight * sigreg_h
        return (1.0 - self.lejepa_lambda) * attract + self.lejepa_lambda * sigreg_h
