"""
Muon-baseline ablation with a tied residual-manifold metric decoder.

Decoder:
- token scores are negative squared distances in a shared warped manifold
- both hidden states and token centers pass through the same residual projector
- token centers stay tied to the base embedding table

Training objective:
- direct metric-CE/NLL by default
- optional TFSD loss on the warped metric logits
"""
...
class WarpedTFSDMetricHeadGPT(base.GPT):
    ...
    def warp(self, x: Tensor) -> Tensor:
        # Shared residual warp keeps the tied geometry as the default and only
        # learns a new output manifold if it provides a better fit.
...
def main() -> None:
    ...
    base_model = WarpedTFSDMetricHeadGPT(args).to(device).bfloat16()
    ...
    log0(
        f"metric_head_warped:tfsd_weight={args.tfsd_weight} metric_ce_weight={args.metric_ce_weight} "
