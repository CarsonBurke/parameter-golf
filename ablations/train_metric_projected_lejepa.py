"""
LeJEPA-inspired ablation: continuous embedding prediction + SIGReg on tok_emb.

Training objective:
- Model outputs h, a model_dim vector per position.
- Loss is MSE between h and the target token's embedding tok_emb[target_id].
  This is literal "predict the embedding" — no vocab softmax, no partition
  function over tokens. The action space is continuous R^model_dim.
- SIGReg on tok_emb.weight prevents token embeddings from collapsing into
  similarity (pushes marginal distribution of rows toward isotropic Gaussian;
  independent Gaussians are near-orthogonal in high dim). This is what lets
  MSE-attract converge without rank-collapsing the target side, which
  defeated r1-r11.

Eval:
- BPB is a next-token-prediction benchmark, so eval has to produce a
  distribution over vocab. We use the standard tied-embedding decoder
  (logits = h @ tok_emb.T, softcap, softmax, CE). At eval time only.
"""
...
class Hyperparameters(base.Hyperparameters):
    # SIGReg weight. At SIGReg steady state, tok_emb rows have per-dim std 1
    # (N(0,I) marginals), so attract = MSE(h, tok_emb[target]) sits around
    # 1-2 per element (h is also ~unit-std per element out of final_norm).
    # SIGReg value starts at ~0.5 and drops to ~0.01 as rows spread out.
    # Weight 1.0 keeps them balanced and matches r13 which proved SIGReg
    # does real work at this weight.
    ...
    # Start tok_emb at per-element std matching h's per-element std. The
    # trunk's final RMSNorm pins h to per-element std ~1 (row norm sqrt(D)),
    # and SIGReg's N(0,I) target has per-element std = 1. So std=1.0 is
    # the only consistent init: attract isn't fighting a scale mismatch,
    # and SIGReg's loss is already near zero at init instead of having to
    # blow up the table by 200x to reach its target. Input-side rms_norm
    # normalizes scale out of the trunk input, so this is safe upstream.
...
class LeJEPAMetricGPT(base.GPT):
    ...
    def sigreg_loss(self, points: Tensor) -> Tensor:
        # Epps-Pulley statistic with Gaussian w(t)=exp(-t²/2) weighting.
        # Non-N-scaled: per-direction mean integral. For 1024 token rows this
        # gives O(0.1-1) regardless of vocab size.
    ...
    def resample_sigreg_dirs(self, step: int) -> None:
        ...
        # Sample in float32 then cast: randn-bfloat16 from a CUDA generator
        # has had version-dependent correctness quirks.
    ...
    def encode_hidden(self, input_ids: Tensor) -> Tensor:
        # final_norm still omitted; L2-norm applied explicitly downstream
        # via F.normalize on the projected geometry, which is both what the
        # sibling does and lets SIGReg operate on the sphere (sphere's
        # random marginals converge to N(0,1) in high dim by concentration
        # of measure, so SIGReg's N(0,I) target is still reachable).
    ...
    def eval_ce_loss(self, hidden: Tensor, target_ids: Tensor) -> Tensor:
        # Standard tied-embedding vocab CE for BPB metric only. Training
        # optimizes continuous MSE(h, tok_emb[target]); eval is the vocab
        # softmax required by the benchmark, computed against raw tok_emb.
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Continuous embedding prediction (r16 config):
        #   attract = MSE(h, tok_emb[target])      # raw Euclidean, no L2-norm
        # SIGReg on both raw h and raw tok_emb, averaged. No projector, no
        # sphere normalization — literal "predict the embed" with anti-
        # collapse on both sides of the attract.
        flat_h = hidden.reshape(-1, hidden.size(-1))
        target_embed = self.tok_emb(target_ids).reshape(-1, hidden.size(-1))
        attract = (flat_h - target_embed).pow(2).mean()
        sigreg = 0.5 * (self.sigreg_loss(flat_h) + self.sigreg_loss(self.tok_emb.weight))
