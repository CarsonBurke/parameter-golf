"""
LeJEPA r53: literal port of CALM's Energy Score loss.

CALM (arXiv 2510.27688) replaces softmax CE with a strictly proper scoring
rule, the Energy Score:

  ℒ_ES(P, y) = 2·E_{X~P}‖X - y‖  −  E_{X,X'~P}‖X - X'‖

Estimator with N model samples and M target samples per position:

  ℒ ≈ (2/(N·M)) Σ_{n,m} ‖z̃_n − z_m‖  −  (1/(N(N-1))) Σ_{n≠k} ‖z̃_n − z̃_k‖

For us: target is a discrete token, so the target distribution is a point
(M=1) at the tied embedding of y — we keep detach on the target side as
CALM does (its VAE is frozen; here we detach an equivalent live codebook
row). To sample from P(·|context), we add a noise-conditioned generator
head that draws N=NUM_ES_SAMPLES samples per position:

  z̃_n = noise_head(hidden, ε_n),   ε_n ~ N(0, I)

What the three pieces do mechanically:
- **L2 distance (α=1)** is what makes ES strictly proper. Not cosine, not
  MSE — squared distance collapses to mean + variance and isn't proper.
- **Attract term** (positive, 2·||·||): pulls each model sample toward
  the (detached) target embedding.
- **Diversity term** (negative, ||z̃_n - z̃_k||): pushes model samples
  at the same position apart. This is the *anti-collapse mechanism* and
  it lives entirely on the predictor side — it penalizes predictor-sample
  collapse regardless of what the target is. The codebook is left alone.

  hidden = encode_hidden(x)                             # [B, T, D]
  eps    = randn(B, T, N, d_noise)
  z̃      = noise_head(hidden, eps)                      # [B, T, N, D]
  e_y    = tok_emb(target_ids).detach()                 # [B, T, D]
  attract    = 2 · mean_n ‖z̃_n − e_y‖
  diversity  = mean_{n≠k} ‖z̃_n − z̃_k‖
  loss       = (attract − diversity).mean()             # per-position ES

Codebook training signal comes from input-side embedding lookup only
(detached on the prediction side, same as CALM's frozen VAE). SIGReg on
the codebook is dropped; pred SIGReg retained as a small guardrail.

Noise head is a tiny MLP: (hidden ⊕ eps_proj) → SwiGLU → proj back to D.
Adds ~D² params (~256K at D=512). Keeps total well under 16MB.

Eval: unchanged — `eval_temp * z @ codebook.T`, where z here is the mean
of the N model samples (i.e. a deterministic forward with eps=0).

Hyperparameters (env):
  LEJEPA_NUM_ES_SAMPLES     — N, default 4
  LEJEPA_NOISE_DIM          — d_noise, default 64
  LEJEPA_ES_NOISE_STD       — scale of eps, default 1.0
  LEJEPA_PRED_SIGREG_WEIGHT — pred guardrail, default 0.01
...
class LeJEPAMetricGramGPT(base.GPT):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Detach target side: CALM-style frozen target (VAE analog). Codebook
        # receives gradient only via input-side embedding lookup.
        tgt_emb = self.tok_emb(tgt_flat).detach()
        ...
        #   ES = 2 · mean_n ||z̃_n − e_y||  −  mean_{n≠k} ||z̃_n − z̃_k||
        # Epsilon floor inside norms to keep autograd stable at coincident
        # points. eps is tiny so it does not bias the estimator.
        eps = self.es_eps
        # attract term: each sample vs (single) target
        diff_target = z_samples - tgt_emb.unsqueeze(-2).to(z_samples.dtype)  # [N_batch, N, D]
        attract_dist = torch.sqrt(diff_target.pow(2).sum(dim=-1) + eps)  # [N_batch, N]
        ...
        if n >= 2:
            ...
            # Mask diagonal; we want n != k. Normalize by n*(n-1).
        ...
        # Diagnostic: cosine alignment of the deterministic sample with target.
        with torch.no_grad():
        ...
        return es_loss + self.pred_sigreg_weight * sigreg_pred
...
def main() -> None:
    ...
    log0(
        ...
        f"head=calm_energy_score_detached_target"
    )
    log0(
