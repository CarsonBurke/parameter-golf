...
class RawEPSIGRegMetricGPT(base.GPT):
    ...
    def residual_geometry(self, hidden: Tensor, target_ids: Tensor) -> Tensor:
        flat_hidden, centers = self.projected_geometry(hidden)
        target = target_ids.reshape(-1)
        target_centers = centers.index_select(0, target).detach()
        return flat_hidden - target_centers
    ...
    def manifold_diagnostics(self, input_ids: Tensor, target_ids: Tensor) -> dict[str, float]:
        ...
        target = target_ids.reshape(-1)
        target_centers = centers.index_select(0, target)
        residual = hidden - target_centers
        ...
        residual_dim_std_mean = residual.std(dim=0, correction=0).mean()
        residual_sq_mean = residual.pow(2).sum(dim=-1).mean()
        return {
            ...
            "residual_dim_std_mean": float(residual_dim_std_mean.item()),
            "residual_sq_mean": float(residual_sq_mean.item()),
        }
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        residual = self.residual_geometry(hidden, target_ids)
        sigreg_loss = residual.new_zeros(())
        loss = self.manifold_attract_weight * attract_loss
        if self.manifold_sigreg_weight > 0.0:
            sigreg_loss = self.sigreg_loss(residual)
...
def main() -> None:
    ...
    log0(
        f"metric_head_projected_residual_sigreg:metric_gamma={base_model.metric_gamma:.4f} "
    ...
    while True:
        ...
        if (step % args.val_loss_every == 0 or last_step) and diag_x is not None and diag_y is not None:
            ...
            log0(
                ...
                f"residual_dim_std_mean:{diag['residual_dim_std_mean']:.4f} residual_sq_mean:{diag['residual_sq_mean']:.4f}"
            )
    ...
    if master_process:
        ...
        int8_bytes = None
        int8_size = 0
        if hasattr(base, "make_model_header"):
