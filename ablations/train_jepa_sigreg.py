"""
JEPA + SIGReg ablation built from the upstream baseline without modifying train_gpt.py.

Training objective:
- predict the next latent from the current latent
- regularize the latent distribution toward isotropic Gaussian statistics
- optionally add a lightweight CE anchor

Eval path:
- identical to the baseline LM path for val_loss / val_bpb
- training-only projector / predictor are excluded from the exported artifact
"""
...
class Hyperparameters(base.Hyperparameters):
    jepa_projector = bool(int(os.environ.get("JEPA_PROJECTOR", "1")))
    jepa_proj_dim = int(os.environ.get("JEPA_PROJ_DIM", os.environ.get("MODEL_DIM", "512")))
    jepa_pred_expansion = int(os.environ.get("JEPA_PRED_EXPANSION", "2"))
    jepa_sigreg_weight = float(os.environ.get("JEPA_SIGREG_WEIGHT", "0.1"))
    jepa_sigreg_projections = int(os.environ.get("JEPA_SIGREG_PROJECTIONS", "32"))
    jepa_sigreg_eps = float(os.environ.get("JEPA_SIGREG_EPS", "1e-4"))
    jepa_stopgrad = bool(int(os.environ.get("JEPA_STOPGRAD", "0")))
    jepa_ce_anchor = float(os.environ.get("JEPA_CE_ANCHOR", "0.0"))
...
class JEPAPredictor(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        return x + self.proj(torch.relu(self.fc(x)))


class JEPAGPT(base.GPT):
    training_only_prefixes = ("projector.", "predictor.", "sigreg_dirs")
    ...
    def __init__(self, args: Hyperparameters):
        ...
        self.jepa_stopgrad = args.jepa_stopgrad
        self.jepa_sigreg_weight = args.jepa_sigreg_weight
        self.jepa_sigreg_eps = args.jepa_sigreg_eps
        self.jepa_ce_anchor = args.jepa_ce_anchor
        latent_dim = args.jepa_proj_dim if args.jepa_projector else args.model_dim
        self.projector = (
            base.CastedLinear(args.model_dim, latent_dim, bias=False)
            if args.jepa_projector
            else Identity()
        )
        self.predictor = JEPAPredictor(latent_dim, args.jepa_pred_expansion)
        dirs = torch.randn(args.jepa_sigreg_projections, latent_dim, dtype=torch.float32)
    ...
    def ce_loss(self, hidden: Tensor, target_ids: Tensor) -> Tensor:
    ...
    def sigreg_loss(self, latent: Tensor) -> Tensor:
        ...
        proj_var = proj_centered.pow(2).mean(dim=0).clamp_min(self.jepa_sigreg_eps)
        proj_std = proj_var.sqrt()
        standardized = proj_centered / proj_std
        skew = standardized.pow(3).mean(dim=0)
        kurt = standardized.pow(4).mean(dim=0)
        mean_loss = proj_mean.pow(2).mean()
        var_loss = (proj_var - 1.0).pow(2).mean()
        skew_loss = skew.pow(2).mean()
        kurt_loss = (kurt - 3.0).pow(2).mean()
    ...
    def training_loss(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.jepa_stopgrad:
            latent_tgt = latent_tgt.detach()
        pred = self.predictor(latent_in)
        pred_loss = F.mse_loss(pred, latent_tgt, reduction="mean")
        sigreg = self.sigreg_loss(latent)
        loss = pred_loss + self.jepa_sigreg_weight * sigreg
        if self.jepa_ce_anchor > 0.0:
            ce = self.ce_loss(hidden, target_ids)
            loss = loss + self.jepa_ce_anchor * ce
        return loss
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        hidden = self.encode_hidden(input_ids)
        return self.ce_loss(hidden, target_ids)
...
def main() -> None:
    ...
    base_model = JEPAGPT(args).to(device).bfloat16()
    ...
    log0(
        f"jepa:projector={int(args.jepa_projector)} proj_dim={args.jepa_proj_dim} "
        f"pred_expansion={args.jepa_pred_expansion} sigreg_weight={args.jepa_sigreg_weight} "
        f"sigreg_projections={args.jepa_sigreg_projections} stopgrad={int(args.jepa_stopgrad)} "
        f"ce_anchor={args.jepa_ce_anchor}"
    )
    log0(
    ...
    allowed_missing = {
        name for name, _ in base_model.named_parameters() if name.startswith(("projector.", "predictor."))
    }
