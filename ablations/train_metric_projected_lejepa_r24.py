"""
LeJEPA / LeWM analog r24: r22 with attract switched from MSE to full-vocab
InfoNCE (cross-entropy over the tied embedding table).

Motivation: MSE = Gaussian NLL with fixed identity covariance. It pulls
flat_h toward the target embedding but never pushes it *away* from wrong
embeddings, can't express multi-modal next-token uncertainty, and has no
learned scale. Full-vocab InfoNCE fixes all three: it's a proper normalized
density over the vocab with an explicit push-away term (the log-partition).

JEPA preservation: tok_emb.weight is detached inside the attract path, so
the embedding table's only gradient sources remain (1) input-side trunk and
(2) sigreg_tok — identical to r22. The only structural change is the attract
term. This makes r24 a like-for-like test of "MSE vs contrastive CE on
continuous next-embedding prediction" holding everything else constant.

The training head is now identical to the eval head (softcap tanh + vocab
CE). Cost: adds one [N, V, D] matmul per micro-step (same FLOPs as the
baseline tied LM head). Expected ~8–15% step-time overhead on 8×H100.

Note on sigreg_weight: attract was ~1.0-scale under MSE. InfoNCE starts near
log(V) ≈ 10.4 and drops to ~3–5, so the effective weight of sigreg relative
to attract shrinks by roughly an order of magnitude. May want to raise
LEJEPA_SIGREG_WEIGHT (env) if tok_emb scale drift reappears.

Loss:
  attract = CE(softcap · tanh(flat_h @ stopgrad(tok_emb).T / softcap), target)
  sigreg  = Epps-Pulley across tok_emb rows (view-set isotropy)
...
class LeJEPAMetricGPT(base.GPT):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Full-vocab InfoNCE against stop-grad token embeddings. Detaching
        # tok_emb.weight keeps the JEPA contract: attract shapes only the
        # predictor (flat_h); tok_emb's gradient paths stay (1) input-side
        # trunk and (2) sigreg_tok, identical to r22.
        logits = F.linear(flat_h, self.tok_emb.weight.detach())
        softcap = self.logit_softcap
        logits = softcap * torch.tanh(logits / softcap)
        attract = F.cross_entropy(logits.float(), targets, reduction="mean")
