...
class JEPAChunkHierARGPT(base.GPT):
    ...
    def chunk_ce_loss(self, input_ids: Tensor, target_ids: Tensor, latent: Tensor, pred_latent: Tensor, *, detach_pred: bool) -> Tensor:
        ...
        cond_latent = torch.cat((latent[:, :1, :], pred_latent[:, :-1, :]), dim=1)
        if detach_pred:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            return loss
        return self.chunk_ce_loss(input_ids, target_ids, latent, pred_latent, detach_pred=False)
...
def main() -> None:
    ...
    log0(
        f"jepa_chunk:chunk_size={args.chunk_size} latent_dim={args.lewm_latent_dim} pred_weight={args.lewm_pred_weight} "
