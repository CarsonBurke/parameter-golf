...
class WMChunkCEGPT(base.GPT):
    ...
    def __init__(self, args: Hyperparameters):
        ...
        self.init_latent = nn.Parameter(torch.zeros(args.chunk_size, args.wm_latent_dim, dtype=torch.float32))
    ...
    def chunk_tokens(self, hidden: Tensor) -> Tensor:
        bsz, seqlen, dim = hidden.shape
        num_chunks = seqlen // self.chunk_size
        return hidden.reshape(bsz, num_chunks, self.chunk_size, dim)

    def encode_latent(self, hidden: Tensor) -> Tensor:
        tokens = self.chunk_tokens(hidden)
        flat = tokens.reshape(-1, self.chunk_size, tokens.size(-1))
        latent = self.encoder_projector(flat)
        return latent.reshape(tokens.size(0), tokens.size(1), self.chunk_size, latent.size(-1))

    def predict_chunk_latents(self, latent: Tensor) -> Tensor:
        init = self.init_latent.to(dtype=latent.dtype)[None, None, :, :].expand(latent.size(0), 1, self.chunk_size, -1)
        prev = torch.cat((init, latent[:, :-1, :, :]), dim=1)
        flat_prev = prev.reshape(-1, self.chunk_size, prev.size(-1))
        pred = self.predictor_projector(self.predictor(flat_prev))
        return pred.reshape_as(prev)
    ...
    def chunk_ce_loss(self, input_ids: Tensor, target_ids: Tensor, cond_latent: Tensor) -> Tensor:
        ...
        logits = self.lexical_decoder(input_chunks, cond_latent.mean(dim=2))
...
def main() -> None:
    ...
    log0(
        f"wm_chunk_ce_hbw:chunk_size={args.chunk_size} latent_dim={args.wm_latent_dim} "
        f"pool=linear_per_token "
