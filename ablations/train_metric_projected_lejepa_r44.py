"""
LeJEPA r44: cosine-InfoNCE with tightly bounded relative state temperature + weak pred SIGReg.
...
r44 keeps r38's full-vocab cosine InfoNCE head, but upgrades the scalar
temperature into a tightly bounded relative per-state sharpness and adds a
small explicit predictor-side SIGReg term:
...
What changes vs r38:
- Temperature is now state-dependent instead of global, but only as a small
  relative adjustment around the learned global base temperature. Some
  contexts can be slightly sharper and others slightly softer without letting
  the head become a second free logit scale.
- The temperature head is deliberately tiny: one scalar projection from the
  unit-normalized prediction, bounded through tanh and then batch-centered so
  it cannot silently absorb global calibration.
