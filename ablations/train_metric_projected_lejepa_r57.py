"""
LeJEPA r57: SIGReg on BOTH hidden and codebook, no detach.

r56 proved that SIGReg on hidden alone is insufficient when tgt is not
detached: codebook collapsed 317 → 15 eff_rank in 80 steps while hidden
SIGReg (a marginal constraint) stayed happy. Common target tokens pull
their codebook row in a consistent direction, and nothing stopped other
rows from piling onto the same direction.

Fix: anchor BOTH marginals to N(0, I). Hidden and codebook each are
pushed toward an isotropic distribution, so neither side can degenerately
match the other by collapsing. Attract has to move mass *within* the
constraint — the transformer must learn context → correct codebook row
direction, which IS the LM task.

r48/r49 also had codebook SIGReg but paired it with `sigreg_pred`
(hidden) at weight 0.01 = effectively off. Symmetric anchoring is new.
...
r57 recipe:
  ...
  loss    = attract
          + λ_h · SIGReg(hidden)
          + λ_c · SIGReg(tok_emb.weight)        # codebook anchor

Defaults: λ_h = λ_c = 0.05. Same magnitude for both; raw sigreg losses
are around 0.2–0.5 and attract is around 0.3, so weighted contributions
stay on the same order.

No predictor head, no geom, no repulsion.
...
def main() -> None:
    ...
    log0(
        ...
        f"head=lejepa_attract_plus_symmetric_sigreg_nodetach"
    )
    log0(
