"""
LeJEPA r49: softmax-free metric attraction + implicit batch Gram + FULL-codebook SIGReg.

r48 result at 60 steps: catastrophic codebook collapse — tok_eff_rank fell
310 -> 24, cos_off_mean 0.17 -> 0.66, val_bpb 3.04 -> 3.71 and rising.
Root cause: r48 put SIGReg on target embeds sampled by token frequency, so
anti-collapse pressure was concentrated on frequent tokens only; rare-token
rows were dragged inward by the attract term with no opposing force. With V
around 1024 in this repo, applying SIGReg to the full codebook each step is
trivially cheap and covers every row.

r49 is r48 with one change:
  sigreg_tgt (batch-sampled) -> sigreg_codebook (all V rows of tok_emb).

  hidden = encode_hidden(x)                       # [B, T, D]
  z      = F.normalize(hidden, dim=-1)             # unit-sphere prediction
  t      = F.normalize(tok_emb(target_ids), -1)    # unit-sphere target
  attract = (1 - (z * t).sum(-1)).mean()
  (opt.)  G_z = z_K @ z_K.T, G_t = t_K @ t_K.T; geom = huber(G_z, G_t, δ)
  sigreg_pred     = SIGReg(hidden)
  sigreg_codebook = SIGReg(tok_emb.weight)         # all V rows, no sampling
  primary = attract + γ·geom
  loss    = (1-λ)·primary + λ·sigreg_codebook + η·sigreg_pred

Risks:
- If λ=0.05 is still too weak, codebook collapses more slowly but still
  collapses. Sweep via LEJEPA_LAMBDA (e.g., 0.2, 0.3, 0.5) or flip to
  additive mode.
- SIGReg applied to all rows regularizes the uniform-over-vocab marginal,
  which is semantically different from the frequency-weighted one. In high
  dim with N(0,I) target, both should give roughly orthogonal rows; in low
  dim the two targets can meaningfully diverge.

Hyperparameters (env): same as r48, but LEJEPA_LAMBDA now weights
sigreg_codebook (not sigreg_tgt).
  LEJEPA_LAMBDA             — convex weight on sigreg_codebook, default 0.05.
  LEJEPA_SIGREG_WEIGHT      — if >= 0, additive mode.
  LEJEPA_PRED_SIGREG_WEIGHT — additive weight on predictor SIGReg, 0.01.
  LEJEPA_GEOM_WEIGHT, LEJEPA_GEOM_HUBER_DELTA, LEJEPA_GEOM_SUBSET — as r48.
  LEJEPA_EVAL_TEMP          — eval-time logit scale, default 15.0.

Diagnostics: attract, geom, geom_mae, pos_cos, sigreg_pred, sigreg_codebook,
sigreg_h surfaced each step; tok_emb row_norm / eff_rank / cos every
val_loss_every.
...
def main() -> None:
    ...
    log0(
        ...
        f"head=metric_attract_batch_gram_codebook_sigreg"
    )
    log0(
