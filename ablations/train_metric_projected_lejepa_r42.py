"""
LeJEPA r42: sharpened live geometric target matching + full-table SIGReg.
...
Observed failures and r42 hypothesis:
- r40 collapsed by neighborhood flattening, not by low-rank collapse:
  q_tgt entropy stayed near uniform and q_tgt(self) fell to ~1%. That made
  the match loss tiny, gradients vanish, and BPB blow up.
- r41 sharpens q_tgt (lower target_temp) and removes the q·log q term from
  the optimized objective by switching from KL(q||p) to cross-entropy
  H(q, p) = -Σ q log p. The target side is still fully live, but it is no
  longer directly rewarded for becoming high-entropy.
- r41 fixed uniformization but the FULL codebook still clustered over time:
  effective rank fell sharply and off-diagonal cosine rose. r42 therefore
  regularizes both the realized targets and the full codebook so the table
  itself cannot cheaply reshape into broad semantic clumps.
...
Hyperparameters (env):
  ...
  LEJEPA_TARGET_TEMP     — target-neighborhood temperature, default 0.15.
  LEJEPA_SIGREG_*        — projection count / quadrature (paper defaults).

Diagnostics: match, sigreg_tgt, sigreg_tok_fwd, sigreg_h (read-only),
predictor temp, target entropy, and target self-prob surfaced each step;
...
class Hyperparameters(base.Hyperparameters):
    ...
    target_temp = float(os.environ.get("LEJEPA_TARGET_TEMP", "0.15"))
...
class LeJEPAMetricGPT(base.GPT):
    def __init__(self, args: Hyperparameters):
        ...
        self.register_buffer("last_sigreg_tok_fwd", torch.zeros((), dtype=torch.float32), persistent=False)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        match = -(target_probs * pred_log_probs).sum(dim=-1).mean()
        sigreg_tgt = self.sigreg_loss(target_emb)
        sigreg_tok_fwd = self.sigreg_loss(self.tok_emb.weight)
        sigreg_total = 0.5 * (sigreg_tgt + sigreg_tok_fwd)
        ...
        self.last_sigreg_tok_fwd.copy_(sigreg_tok_fwd.detach())
        ...
        if self.use_additive:
            return match + self.sigreg_weight * sigreg_total
        return (1.0 - self.lejepa_lambda) * match + self.lejepa_lambda * sigreg_total
...
def main() -> None:
    ...
    log0(
        ...
        f"head=sharpened_live_geometric_target_xent_fulltok_sigreg"
    )
    log0(
    ...
    while True:
        ...
        if step % 10 == 0 or last_step:
            log0(
                ...
                f"sigreg_tok_fwd:{base_model.last_sigreg_tok_fwd.item():.4f} "
