"""
Muon-baseline ablation with a normalized TFSD metric decoder.

This stays close to the TFSD post:
- one centroid per token
- metric logits are negative squared distances
- pure TFSD training, no CE anchor by default
- explicit per-vector normalization of both hidden states and token centers before distance
"""
...
class Hyperparameters(base.Hyperparameters):
    ...
    metric_eps = float(os.environ.get("TFSD_METRIC_EPS", "1e-6"))


class NormalizedTFSDMetricHeadGPT(base.GPT):
    def __init__(self, args: Hyperparameters):
        ...
        self.metric_eps = args.metric_eps
    ...
    def normalize_vectors(self, x: Tensor) -> Tensor:
        x = x.float()
        return F.rms_norm(x, (x.size(-1),), eps=self.metric_eps)

    def metric_logits(self, hidden: Tensor) -> Tensor:
        flat_hidden = self.normalize_vectors(hidden.reshape(-1, hidden.size(-1))) * self.metric_input_scale
        centers = self.normalize_vectors(self.token_centers()) * self.metric_input_scale
...
def main() -> None:
    ...
    base_model = NormalizedTFSDMetricHeadGPT(args).to(device).bfloat16()
    ...
    log0(
        f"normalized_metric:tfsd_weight={args.tfsd_weight} metric_ce_weight={args.metric_ce_weight} "
        f"teacher_temp={args.tfsd_teacher_temp} student_temp={args.tfsd_student_temp} "
        f"metric_gamma={base_model.metric_gamma:.4f} metric_eps={args.metric_eps}"
    )
    log0(
