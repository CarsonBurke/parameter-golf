"""
Muon-baseline ablation with a tied shared-manifold objective.

Decoder:
- hidden states and token centers live in the same residual output manifold
- token centers remain tied to the base embedding table
- eval still uses dense metric decoding for BPB

Training objective:
- attract the predicted point to the correct token center
- regularize the live manifold geometry with SIGReg to prevent collapse
- optional light SIGReg on predictions; no CE by default
"""
...
class Hyperparameters(base.Hyperparameters):
    ...
    manifold_z_sigreg_weight = float(os.environ.get("MANIFOLD_Z_SIGREG_WEIGHT", "0.0"))
    manifold_sigreg_projections = int(os.environ.get("MANIFOLD_SIGREG_PROJECTIONS", "32"))
    manifold_sigreg_eps = float(os.environ.get("MANIFOLD_SIGREG_EPS", "1e-4"))


class ManifoldSIGRegMetricGPT(base.GPT):
    def __init__(self, args: Hyperparameters):
        ...
        self.manifold_z_sigreg_weight = args.manifold_z_sigreg_weight
        self.manifold_sigreg_eps = args.manifold_sigreg_eps
        self.output_proj = base.CastedLinear(args.model_dim, args.model_dim, bias=False)
        nn.init.zeros_(self.output_proj.weight)
        dirs = torch.randn(args.manifold_sigreg_projections, args.model_dim, dtype=torch.float32)
    ...
    def warp(self, x: Tensor) -> Tensor:
        return x + self.output_proj(x)

    def metric_logits(self, hidden: Tensor) -> Tensor:
        warped_hidden = self.warp(hidden)
        warped_centers = self.warp(self.token_centers())
        flat_hidden = warped_hidden.reshape(-1, warped_hidden.size(-1)).float() * self.metric_input_scale
        centers = warped_centers.float() * self.metric_input_scale
    ...
    def sigreg_loss(self, points: Tensor) -> Tensor:
        ...
        proj_var = proj_centered.pow(2).mean(dim=0).clamp_min(self.manifold_sigreg_eps)
    ...
    def manifold_attract_loss(self, hidden: Tensor, target_ids: Tensor) -> Tensor:
        warped_hidden = self.warp(hidden).reshape(-1, hidden.size(-1)).float()
        warped_centers = self.warp(self.token_centers()).float()
        target = target_ids.reshape(-1)
        target_centers = warped_centers.index_select(0, target)
        delta = warped_hidden - target_centers
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        warped_centers = self.warp(self.token_centers())
        loss = self.manifold_attract_loss(hidden, target_ids)
        if self.manifold_sigreg_weight > 0.0:
            loss = loss + self.manifold_sigreg_weight * self.sigreg_loss(warped_centers)
        if self.manifold_z_sigreg_weight > 0.0:
            warped_hidden = self.warp(hidden)
            loss = loss + self.manifold_z_sigreg_weight * self.sigreg_loss(warped_hidden)
        return loss
...
def main() -> None:
    ...
    base_model = ManifoldSIGRegMetricGPT(args).to(device).bfloat16()
    ...
    log0(
        f"metric_head_manifold_sigreg:metric_gamma={base_model.metric_gamma:.4f} "
        f"sigreg_weight={args.manifold_sigreg_weight} z_sigreg_weight={args.manifold_z_sigreg_weight} "
