"""
LeJEPA r47: semantic-aware full-token-geometry contrast + SIGReg.
...
r47 keeps the same "predict one continuous embed z" trunk as r44, but stops
optimizing a scalar token score directly. Instead, it matches the FULL token
geometry induced by z against the full token geometry of the true target row,
while only enforcing strong separation where the target row says a negative is
actually far:

  hidden      = encode_hidden(x)                              # [B, T, D]
  z           = F.normalize(hidden, dim=-1)                   # unit-sphere
  codebook    = F.normalize(tok_emb.weight, dim=-1)           # unit-sphere
  g_pred      = z @ codebook.T                                # [N, V]
  g_tgt       = codebook[target_ids] @ codebook.T             # [N, V]
  geom        = huber(g_pred, g_tgt)                          # full token geometry
  pos         = g_pred[target_ids]                            # predicted true-token sim
  w_v         = clamp(1 - g_tgt[v], min=0)                    # repel far tokens harder
  m_v         = m0 + ms * clamp(1 - g_tgt[v], min=0)          # larger margin for far tokens
  rank        = mean_v w_v * relu(g_pred[v] - pos + m_v)      # semantic-aware repel
  ...
  primary     = geom + β·rank
  loss        = (1-λ)·primary + λ·sigreg_tgt + η·sigreg_pred

What changes vs r44:
- The learning signal is no longer "make the true token score high under a
  softmax over scalar scores." Instead, z must reproduce the true token's
  entire similarity row against the live codebook.
- This is still full-vocab attract/push, but it acts on the whole token
  geometry: dog should be close to dog, somewhat close to cat, far from car,
  etc.
- Repulsion is semantic-aware rather than uniform. Nearby negatives with high
  target-row similarity get small weight and small margin; distant negatives
  get pushed away much harder.
- There is no learned temperature or state-dependent scalar head in the train
  objective. The geometry itself is the target.
- r44's weak pred-side SIGReg is retained as a stabilizer, and target-side
  SIGReg is unchanged.
...
Risks vs r44:
- This can underperform on BPB even if the geometry gets better, because the
  training loss is no longer directly optimizing a categorical softmax.
- If the codebook geometry collapses, the row-matching objective becomes too
  easy. Target-side SIGReg is meant to counteract that.
- If the semantic-aware repel term is too weak, rows may match semantically
  but the true token may still not win sharply enough at decode time.
- If the semantic-aware repel term is too strong, the loss can quietly drift
  back toward uniform negative pushing and destroy useful local neighborhoods.
...
Hyperparameters (env):
  ...
  LEJEPA_PRED_SIGREG_WEIGHT — additive weight on predictor-side SIGReg,
                              default 0.01.
  LEJEPA_GEOM_HUBER_DELTA   — Huber delta for full-row geometry matching,
                              default 0.1.
  LEJEPA_RANK_WEIGHT        — additive weight on semantic-aware repel term,
                              default 1.0.
  LEJEPA_RANK_MARGIN_BASE   — minimum margin against negatives, default 0.02.
  LEJEPA_RANK_MARGIN_SCALE  — extra margin for semantically far negatives,
                              default 0.20.

Diagnostics: geom, rank, geom_mae, pos_cos, sigreg_tgt, sigreg_pred, sigreg_h
surfaced each step; tok_emb row_norm / eff_rank / cos every val_loss_every.

Eval: deploy the raw geometry row `z @ codebook.T` as logits and measure CE /
Bpb on that deployed decoder. That keeps evaluation honest to the trained
representation, even though the train loss is not itself softmax CE.
"""
...
class Hyperparameters(base.Hyperparameters):
    ...
    rank_weight = float(os.environ.get("LEJEPA_RANK_WEIGHT", "1.0"))
    rank_margin_base = float(os.environ.get("LEJEPA_RANK_MARGIN_BASE", "0.02"))
    rank_margin_scale = float(os.environ.get("LEJEPA_RANK_MARGIN_SCALE", "0.20"))


class LeJEPASemanticGeometryGPT(base.GPT):
    def __init__(self, args: Hyperparameters):
        ...
        self.rank_weight = float(args.rank_weight)
        self.rank_margin_base = float(args.rank_margin_base)
        self.rank_margin_scale = float(args.rank_margin_scale)
        ...
        self.register_buffer("last_rank", torch.zeros((), dtype=torch.float32), persistent=False)
    ...
    def geometry_rows(self, flat_hidden: Tensor, target_ids: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        z = F.normalize(flat_hidden.float(), dim=-1)
        codebook = F.normalize(self.tok_emb.weight.float(), dim=-1)
        pred_geom = z @ codebook.T
        target_unit = codebook[target_ids]
        target_geom = target_unit @ codebook.T
        pos_cos = (z * target_unit).sum(dim=-1)
        return pred_geom, target_geom, pos_cos
    ...
    def eval_ce_loss(self, hidden: Tensor, target_ids: Tensor) -> Tensor:
        ...
        logits, _, _ = self.geometry_rows(flat, tgt_flat)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Full token-geometry matching: z must reproduce the true token row in
        # the live cosine-similarity matrix over the codebook.
        flat_h = hidden.reshape(-1, hidden.size(-1))
        tgt_flat = target_ids.reshape(-1)
        pred_geom, target_geom, pos_cos = self.geometry_rows(flat_h, tgt_flat)
        geom = F.huber_loss(pred_geom, target_geom, reduction="mean", delta=self.geom_huber_delta)
        pos_pred = pred_geom.gather(1, tgt_flat.unsqueeze(1))
        semantic_far = (1.0 - target_geom).clamp_min(0.0)
        rank_margin = self.rank_margin_base + self.rank_margin_scale * semantic_far
        rank_violation = pred_geom - pos_pred + rank_margin
        non_target_mask = torch.ones_like(rank_violation, dtype=torch.bool)
        non_target_mask.scatter_(1, tgt_flat.unsqueeze(1), False)
        weighted_violation = semantic_far * F.relu(rank_violation) * non_target_mask
        rank = weighted_violation.sum() / non_target_mask.sum().clamp_min(1)
        primary = geom + self.rank_weight * rank
        ...
        with torch.no_grad():
            ...
            geom_mae = F.l1_loss(pred_geom, target_geom, reduction="mean")
            pos_cos_mean = pos_cos.mean()
        ...
        self.last_rank.copy_(rank.detach())
        self.last_geom_mae.copy_(geom_mae.detach())
        self.last_pos_cos.copy_(pos_cos_mean.detach())
        if self.use_additive:
            return primary + self.sigreg_weight * sigreg_tgt + self.pred_sigreg_weight * sigreg_pred
        return (1.0 - self.lejepa_lambda) * primary + self.lejepa_lambda * sigreg_tgt + self.pred_sigreg_weight * sigreg_pred
...
def main() -> None:
    ...
    base_model = LeJEPASemanticGeometryGPT(args).to(device).bfloat16()
    ...
    log0(
        ...
        f"rank_weight={args.rank_weight} "
        f"rank_margin_base={args.rank_margin_base} "
        f"rank_margin_scale={args.rank_margin_scale} "
        f"head=semantic_token_geometry_contrast"
    )
    log0(
    ...
    while True:
        ...
        if step % 10 == 0 or last_step:
            log0(
                ...
                f"rank:{base_model.last_rank.item():.4f} "
