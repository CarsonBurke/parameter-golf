"""
LeWM-inspired latent-prediction ablation built from the upstream baseline without modifying train_gpt.py.

Training objective:
- standard next-token CE on the trunk hidden states
- latent prediction MSE on a training-only projector branch
- SIGReg-style anti-collapse regularization on the latent branch

The latent branch is detached from the lexical head in the architectural sense: the CE head reads the
baseline hidden states only, while the projector/predictor branch is training-only and excluded from export.
"""
...
class LeWMLatentGPT(base.GPT):
    ...
    def ce_from_hidden(self, hidden: Tensor, target_ids: Tensor, *, detach_hidden: bool = False) -> Tensor:
        if detach_hidden:
    ...
    def sigreg_loss(self, latent: Tensor) -> Tensor:
        ...
        weight = target
        integrand = ((real - target).pow(2) + imag.pow(2)) * weight
    ...
    def training_loss(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.direct_ce_weight > 0.0:
            loss = loss + self.direct_ce_weight * self.ce_from_hidden(hidden, target_ids, detach_hidden=True)
...
def main() -> None:
    ...
    base_model = LeWMLatentGPT(args).to(device).bfloat16()
    ...
    log0(
        f"lewm_latent:direct_ce_weight={args.direct_ce_weight} latent_dim={args.lewm_latent_dim} "
        f"pred_weight={args.lewm_pred_weight} sigreg_weight={args.lewm_sigreg_weight} "
        f"pred_expansion={args.lewm_pred_expansion} pred_dropout={args.lewm_pred_dropout} "
    ...
    if any(
        not any(name.startswith(prefix) for prefix in base_model.training_only_prefixes)
        for name in missing
    ):
