"""
Hierarchical JEPA-first LM:

- encoder maps the full token prefix to token hidden states
- hidden states are pooled into slow chunk latents z_k
- predictor rolls z_k forward to future chunk latents z'_{k+1}
- a fast lexicalizer reads only local token history inside each target chunk plus the predicted slow latent
  for that chunk, and emits exact next-token logits

This makes JEPA the slow state model while the AR decoder acts as a serializer of predicted state.
"""
...
class Hyperparameters(base.Hyperparameters):
    ...
    lewm_pred_horizon = int(os.environ.get("LEWM_PRED_HORIZON", "4"))
    ...
    lewm_sigreg_stride = int(os.environ.get("LEWM_SIGREG_STRIDE", "8"))
    ...
    ar_decoder_heads = int(os.environ.get("AR_DECODER_HEADS", "8"))
    ar_decoder_kv_heads = int(os.environ.get("AR_DECODER_KV_HEADS", "4"))
...
class JEPAChunkHierARGPT(base.GPT):
    ...
    def __init__(self, args: Hyperparameters):
        ...
        if args.ar_decoder_heads > args.model_dim or args.model_dim % args.ar_decoder_heads != 0:
            raise ValueError("ar_decoder_heads must divide model_dim")
        self.chunk_size = args.chunk_size
        self.lewm_pred_weight = args.lewm_pred_weight
        self.lewm_pred_horizon = args.lewm_pred_horizon
    ...
    def rollout_cond_latents(self, latent: Tensor) -> Tensor:
        num_chunks = latent.size(1)
        rollout = [latent[:, :1, :]]
        current = latent[:, :1, :]
        for _ in range(1, num_chunks):
            current = self.predict_future_latent(current)
            rollout.append(current)
        return torch.cat(rollout, dim=1)

    def pred_loss(self, latent: Tensor) -> Tensor:
        max_h = min(self.lewm_pred_horizon, latent.size(1) - 1)
        if max_h <= 0:
            return latent.new_zeros(())
        total = latent.new_zeros(())
        current = latent
        for horizon in range(1, max_h + 1):
            current = self.predict_future_latent(current[:, :-1, :])
            total = total + F.mse_loss(current, latent[:, horizon:, :], reduction="mean")
        return total / max_h
    ...
    def chunk_ce_loss(self, input_ids: Tensor, target_ids: Tensor, cond_latent: Tensor, *, detach_pred: bool) -> Tensor:
        ...
        if detach_pred:
            cond_latent = cond_latent.detach()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        rollout_latent = self.rollout_cond_latents(latent)
        if self.training:
            ...
            if self.ar_ce_weight > 0.0:
                loss = loss + self.ar_ce_weight * self.chunk_ce_loss(
                    input_ids,
                    target_ids,
                    rollout_latent,
            ...
            return loss
        return self.chunk_ce_loss(input_ids, target_ids, rollout_latent, detach_pred=False)
...
def main() -> None:
    ...
    base_model = JEPAChunkHierARGPT(args).to(device).bfloat16()
    ...
    log0(
        f"jepa_chunk_rollout:chunk_size={args.chunk_size} latent_dim={args.lewm_latent_dim} pred_weight={args.lewm_pred_weight} "
        f"pred_horizon={args.lewm_pred_horizon} "
        f"sigreg_weight={args.lewm_sigreg_weight} pred_expansion={args.lewm_pred_expansion} pred_dropout={args.lewm_pred_dropout} "
        f"sigreg_projections={args.lewm_sigreg_projections} sigreg_stride={args.lewm_sigreg_stride} "
        f"ce_weight={args.ar_ce_weight} decoder_layers={args.ar_decoder_layers} decoder_heads={args.ar_decoder_heads} "
