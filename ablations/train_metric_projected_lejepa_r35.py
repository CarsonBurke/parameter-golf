"""
LeJEPA r35: CE + SIGReg on encoder output. Minimum-diff paper-faithful port
of SIGReg to LM, with tied-head CE kept as the attract/discriminator.

  hidden    = encode_hidden(x)             # [B, T, D], post-final-norm
  flat_h    = hidden.reshape(-1, D)
  logits    = softcap · tanh(flat_h @ tok_emb.T / softcap)
  ce        = cross_entropy(logits, targets)      # standard tied-head CE
  sigreg_h  = Epps-Pulley on flat_h vs N(0, I)    # paper-faithful SIGReg
  loss      = (1-λ)·ce + λ·sigreg_h               # λ=0.05 paper default

Why this is the right sober test of SIGReg for LM:
- Tied-head CE = full-vocab InfoNCE with tok_emb as codebook. Both sides
  (flat_h AND tok_emb.weight) get gradient from CE — the same two-sided
  property vision LeJEPA gets from MSE between two encoder outputs. So CE
  IS the "attract" term here, and it already works (baseline 1.30 BPB
  at step 2000).
- r22–r34 all weakened this signal (detach on tok_emb, attract swapped
  to MSE/shallow target, SIGReg on tok_emb.weight fighting CE's frequency-
  anisotropic shaping) and all landed 2.5–3× worse than CE. r35 removes
  every one of those departures and asks the cleanest question: does
  SIGReg on the encoder-output distribution improve CE at all?

Differences from prior attempts:
- r21 (SIGReg on flat_h + MSE attract, detach): SIGReg fought attract on
  flat_h's scale. Here CE's pull on flat_h is gentler (logit-magnitude,
  not squared-error-magnitude), so the per-dim-std=1 target of SIGReg
  should be compatible.
- r22–r30 (SIGReg on tok_emb.weight or mixed views): placed regularizer on
  a different tensor than attract acted on, with a uniform-vs-frequency
  conflict. r35 puts SIGReg on flat_h only — same tensor CE pulls — so
  attraction and regularization speak the same distribution.
- No detach, no predictor, no projector, no second forward. This is
  "baseline + one loss term".

Known non-risks (because CE prevents them):
- Codebook collapse: impossible — CE gives output-side gradient to
  tok_emb, pulling rows toward contexts that predict them. tok_emb cannot
  collapse while CE is on.
- BPB alignment: eval uses the exact tied-head CE used in training, no
  gap.

Known real risks:
- SIGReg's N(0,1) per-dim marginal target may fight CE's tendency to let
  hidden magnitudes grow along frequency-dominant directions. Watch
  tok_row_norm_std and tok_cos_off_mean — if they drift adversarially
  vs baseline, λ is too high.
- Loss-scale mismatch near init. CE starts ~log(V) ≈ 10.4, SIGReg starts
  O(0.1–1.0). The convex form (1-λ)·ce + λ·sigreg_h with λ=0.05 keeps CE
  dominant; additive mode (LEJEPA_SIGREG_WEIGHT ≥ 0) lets us sweep
  independently.

Hyperparameters (env):
  LEJEPA_LAMBDA          — convex weight on sigreg, default 0.05.
  LEJEPA_SIGREG_WEIGHT   — if >= 0, use additive: loss = ce + w·sigreg_h.
                           Default -1 (disabled; convex mode is used).
  LEJEPA_SIGREG_*        — projection count / quadrature (paper defaults).

Diagnostics: unchanged from r34 — sigreg_tok, tok_row_norm, tok_eff_rank,
etc. logged every val_loss_every to catch tok_emb pathologies early.

Eval: tied-head CE for BPB; identical to train-time CE term.
"""
...
class LeJEPAMetricGPT(base.GPT):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Tied-head CE. This is the "attract" term: full-vocab InfoNCE with
        # tok_emb as codebook. Gradient flows into flat_h (decoder input)
        # AND into tok_emb (decoder prototypes) — same two-sided property
        # vision LeJEPA gets from MSE between two encoder outputs, which is
        # what makes CE the right LM analog of the paper's L_pred.
        ...
        # SIGReg on the full encoder-output distribution (all positions,
        # all sequences). This is the "SIGReg per encoder view z" from the
        # paper — same tensor flat_h that CE operates on, so regularizer
        # and attract speak the same distribution (no uniform-vs-frequency
        # conflict like r22–r30 had when SIGReg lived on tok_emb.weight).
        sigreg_h = self.sigreg_loss(flat_h)

        self.last_ce.copy_(ce.detach())
        self.last_sigreg_h.copy_(sigreg_h.detach())
        if self.use_additive:
            return ce + self.sigreg_weight * sigreg_h
        return (1.0 - self.lejepa_lambda) * ce + self.lejepa_lambda * sigreg_h
