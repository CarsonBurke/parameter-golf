...
class Hyperparameters(base.Hyperparameters):
    ...
    sequence_sigreg_decay = float(os.environ.get("SEQUENCE_SIGREG_DECAY", "0.99"))
...
class RawEPSIGRegMetricGPT(base.GPT):
    def __init__(self, args: Hyperparameters):
        ...
        if not 0.0 <= args.sequence_sigreg_decay < 1.0:
            raise ValueError(f"SEQUENCE_SIGREG_DECAY must be in [0, 1), got {args.sequence_sigreg_decay}")
        self.sequence_sigreg_decay = args.sequence_sigreg_decay
        self.sequence_sigreg_input_scale = math.sqrt(max(1e-8, 1.0 - args.sequence_sigreg_decay**2))
    ...
    def projected_tokens(self, hidden: Tensor) -> tuple[Tensor, Tensor]:
        pred = self.projector(hidden).float()
        centers = self.projector_target(self.token_centers()).float()
        return F.normalize(pred, dim=-1), F.normalize(centers, dim=-1)

    def sequence_state(self, hidden: Tensor) -> Tensor:
        token_latents, _ = self.projected_tokens(hidden)
        state = torch.zeros(
            token_latents.size(0),
            token_latents.size(-1),
            device=token_latents.device,
            dtype=token_latents.dtype,
        )
        decay = self.sequence_sigreg_decay
        input_scale = self.sequence_sigreg_input_scale
        for t in range(token_latents.size(1)):
            state = decay * state + input_scale * token_latents[:, t, :]
        return state
    ...
    def manifold_diagnostics(self, input_ids: Tensor, target_ids: Tensor) -> dict[str, float]:
        hidden_3d = self.encode_hidden(input_ids)
        trunk_hidden = hidden_3d.reshape(-1, self.token_centers().size(-1))
        ...
        seq_state = self.sequence_state(hidden_3d).float()
        target = target_ids.reshape(-1)
        target_centers = centers.index_select(0, target)
        ...
        return {
            ...
            "seq_norm_mean": float(seq_norm.mean().item()),
            "seq_norm_std": float(seq_norm.std(correction=0).item()),
            "pred_dim_std_mean": float(pred_dim_std_mean.item()),
            "center_dim_std_mean": float(center_dim_std_mean.item()),
            "seq_dim_std_mean": float(seq_dim_std_mean.item()),
        }
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.manifold_sigreg_weight > 0.0:
            loss = loss + self.manifold_sigreg_weight * self.sigreg_loss(self.sequence_state(hidden))
        return loss
...
def main() -> None:
    ...
    log0(
        f"metric_head_projected_stream_sigreg:metric_gamma={base_model.metric_gamma:.4f} "
        ...
        f"projector_dim={args.projector_dim} seq_decay={args.sequence_sigreg_decay}"
    )
    log0(
    ...
    while True:
        ...
        if (step % args.val_loss_every == 0 or last_step) and diag_x is not None and diag_y is not None:
            ...
            log0(
                ...
                f"seq_norm_mean:{diag['seq_norm_mean']:.4f} seq_norm_std:{diag['seq_norm_std']:.4f} "
                f"pred_dim_std_mean:{diag['pred_dim_std_mean']:.4f} center_dim_std_mean:{diag['center_dim_std_mean']:.4f} "
                f"seq_dim_std_mean:{diag['seq_dim_std_mean']:.4f}"
            )
