"""
Muon-baseline ablation with a deployed entmax TFSD metric decoder.

Decoder:
- token scores are negative squared distances to learned token centers
- token probabilities use entmax-style normalization instead of exp/logsumexp
- the same token table is used at inference, so the geometric regime is deployed rather than auxiliary

Training objective:
- primary TFSD loss on the entmax-normalized metric probabilities
- optional small metric-NLL anchor for stabilization
"""
...
class Hyperparameters(base.Hyperparameters):
    ...
    entmax_alpha = float(os.environ.get("TFSD_ENTMAX_ALPHA", "1.2"))
    entmax_iters = int(os.environ.get("TFSD_ENTMAX_ITERS", "32"))
    prob_floor = float(os.environ.get("TFSD_PROB_FLOOR", "1e-30"))


class EntmaxTFSDMetricHeadGPT(base.GPT):
    def __init__(self, args: Hyperparameters):
        ...
        self.entmax_iters = args.entmax_iters
        self.prob_floor = args.prob_floor
        if self.entmax_alpha <= 1.0:
            raise ValueError(f"TFSD_ENTMAX_ALPHA must be > 1, got {self.entmax_alpha}")
        if self.entmax_iters <= 0:
            raise ValueError(f"TFSD_ENTMAX_ITERS must be positive, got {self.entmax_iters}")
    ...
    def entmax_probs(self, logits: Tensor, temperature: float = 1.0) -> Tensor:
        x = logits.float() / max(temperature, 1e-6)
        x = (self.entmax_alpha - 1.0) * x
        tau_hi = x.max(dim=-1, keepdim=True).values
        tau_lo = tau_hi - 1.0
        power = 1.0 / (self.entmax_alpha - 1.0)
        for _ in range(self.entmax_iters):
            tau_mid = 0.5 * (tau_lo + tau_hi)
            probs_mid = torch.relu(x - tau_mid).pow(power)
            sum_mid = probs_mid.sum(dim=-1, keepdim=True)
            tau_lo = torch.where(sum_mid > 1.0, tau_mid, tau_lo)
            tau_hi = torch.where(sum_mid <= 1.0, tau_mid, tau_hi)
        probs = torch.relu(x - tau_hi).pow(power)
    ...
    def metric_nll_loss(self, hidden: Tensor, target_ids: Tensor) -> Tensor:
        ...
        probs = self.entmax_probs(logits)
        target_flat = target_ids.reshape(-1)
        target_probs = probs.gather(1, target_flat.unsqueeze(1)).squeeze(1)
        return -torch.log(target_probs.clamp_min(self.prob_floor)).mean()

    def tfsd_loss(self, hidden: Tensor, target_ids: Tensor) -> Tensor:
        target_flat = target_ids.reshape(-1)
        student_logits = self.metric_logits(hidden)
        student_probs = self.entmax_probs(student_logits, temperature=self.tfsd_student_temp)
        with torch.no_grad():
            ...
            teacher_probs = self.entmax_probs(teacher_logits, temperature=self.tfsd_teacher_temp)
        return -(teacher_probs * torch.log(student_probs.clamp_min(self.prob_floor))).sum(dim=-1).mean()
...
def main() -> None:
    ...
    base_model = EntmaxTFSDMetricHeadGPT(args).to(device).bfloat16()
    ...
    log0(
        ...
        f"metric_gamma={base_model.metric_gamma:.4f} entmax_alpha={args.entmax_alpha:.4f} "
        f"entmax_iters={args.entmax_iters}"
    )
    log0(
