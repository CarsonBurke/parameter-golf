...
- regularize the live token embedding geometry with SIGReg to prevent collapse
...
class RawSIGRegMetricGPT(base.GPT):
    ...
    def manifold_diagnostics(self, input_ids: Tensor, target_ids: Tensor) -> dict[str, float]:
        ...
        return {
            "gold_dist_mean": float(gold_dist.mean().item()),
            "gold_dist_std": float(gold_dist.std(correction=0).item()),
            "pred_norm_mean": float(pred_norm.mean().item()),
            "pred_norm_std": float(pred_norm.std(correction=0).item()),
            "center_norm_mean": float(center_norm.mean().item()),
            "center_norm_std": float(center_norm.std(correction=0).item()),
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        centers = self.token_centers()
        loss = self.manifold_attract_weight * self.manifold_attract_loss(hidden, target_ids)
        if self.manifold_sigreg_weight > 0.0:
            loss = loss + self.manifold_sigreg_weight * self.sigreg_loss(centers)
        if self.manifold_z_sigreg_weight > 0.0:
            loss = loss + self.manifold_z_sigreg_weight * self.sigreg_loss(hidden)
        return loss
...
def main() -> None:
    ...
    base_model = RawSIGRegMetricGPT(args).to(device).bfloat16()
    ...
    log0(
        f"metric_head_raw_sigreg:metric_gamma={base_model.metric_gamma:.4f} "
    ...
    while True:
        ...
        if (step % args.val_loss_every == 0 or last_step) and diag_x is not None and diag_y is not None:
            ...
            log0(
                ...
                f"pred_norm_mean:{diag['pred_norm_mean']:.4f} pred_norm_std:{diag['pred_norm_std']:.4f} "
                f"center_norm_mean:{diag['center_norm_mean']:.4f} center_norm_std:{diag['center_norm_std']:.4f} "
