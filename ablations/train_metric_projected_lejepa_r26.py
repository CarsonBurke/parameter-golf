"""
LeJEPA / LeWM analog r26: r24 with the target-side stop-grad on tok_emb
removed. CE now gradients into the embedding table directly.

r24 (InfoNCE + detached target + SIGReg) diverged: attract rose past log(V),
val_bpb degraded. r25 (InfoNCE + detached target, no SIGReg) also failed —
attract stuck at log(V), slow drift. Diagnosis: the JEPA stop-grad trick
works for MSE because MSE only needs the target to exist, but CE requires
the embedding table to be *actively shaped by the prediction objective* to
produce discriminable logits. Input-side-trunk gradients alone cannot
provide that shaping. Standard tied LMs rely on the output-logits gradient
path through the tied table — removing it breaks CE.

Change here: drop `.detach()` on `tok_emb.weight` in the attract path. CE
now gradients into the embedding table via the usual tied-LM pathway. This
abandons the pure JEPA framing (there's now no "stopgrad teacher") and
becomes a standard tied-LM + SIGReg combo. We keep SIGReg on tok_emb as a
regularizer to test whether its isotropy pressure helps or hurts when the
predictor can actually shape the table.

Loss:
  attract = CE(softcap · tanh(flat_h @ tok_emb.T / softcap), target)
  sigreg  = Epps-Pulley across tok_emb rows (view-set isotropy)
  total   = attract + sigreg_weight * sigreg

tok_emb gradient sources: (1) input-side trunk, (2) attract via tied head
(no stop-grad), (3) sigreg_tok. Collapse prevented by the push-apart signal
in CE's log-partition plus SIGReg.
...
class LeJEPAMetricGPT(base.GPT):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Full-vocab CE over the tied embedding table WITHOUT stop-grad:
        # tok_emb.weight now receives gradient from the predictor head as
        # well as the input-side trunk and SIGReg. This is the standard
        # tied-LM pathway; r24/r25 showed that CE cannot train without it.
        logits = F.linear(flat_h, self.tok_emb.weight)
        softcap = self.logit_softcap
        logits = softcap * torch.tanh(logits / softcap)
