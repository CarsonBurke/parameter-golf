"""
LeJEPA r54: CALM Energy Score loss with a JITTERED (M>1) target.

Why r53 plateaued: the full ES estimator

  ES ≈ (2/(N·M)) Σ_{n,m} ‖z̃_n − z_m‖  −  (1/(N(N-1))) Σ_{n≠k} ‖z̃_n − z̃_k‖

is a *strictly proper scoring rule for matching distributions*. With M=1
(a single point target), the proper optimum is `z̃_n = z_target` for all
n — the diversity term goes to 0 at that optimum and the loss just rewards
point attraction. But in finite-step practice, the coefficients favor
spreading samples (a marginal increase in spread gains more from diversity
than it loses on attract), so training settles in a degenerate regime:
samples spread around target with no pressure to concentrate. r53 plateaued
at bpb ~2.78 vs r50's 2.26.

Fix: give ES a real distribution to match. Jitter the (detached) target
embedding with isotropic Gaussian noise, producing M>1 target samples:

  z_m = tok_emb(y).detach() + σ · η_m,   η_m ~ N(0, I),  m = 1..M

Now the target distribution is a Gaussian ball of radius σ around e_y. The
proper optimum is for the model's predictive distribution to have the same
spread σ/√3 (in the 2-coefficient CALM normalization), which means samples
naturally converge toward e_y while maintaining calibrated spread — the
diversity term does real work.

Choice of σ: must be much smaller than inter-row distance between codebook
rows (√(2D) ≈ 32 at tied_embed_init_std=1), but large enough for the
distribution to have non-trivial entropy against autograd noise. σ=2.0
default gives SNR (row_pair_dist / σ) ≈ 16 — clearly distinguishable rows,
meaningful target spread.

Loss (per position):
  attract   = 2 · (1/(N·M)) Σ_{n,m} ‖z̃_n − z_m‖
  diversity = (1/(N(N-1))) Σ_{n≠k} ‖z̃_n − z̃_k‖
  es        = attract − diversity

Everything else (generator head, detached codebook target, sigreg_pred
guardrail, deterministic eval via eps=0 sample) stays identical to r53.

Hyperparameters (env): same as r53, plus
  LEJEPA_NUM_TARGET_SAMPLES  — M, default 4
  LEJEPA_TARGET_JITTER_STD   — σ, default 2.0
"""
...
def main() -> None:
    ...
    log0(
        ...
        f"head=calm_energy_score_jittered_target"
    )
    log0(
