"""
Muon-baseline ablation with a projected untied deployed metric decoder.

Decoder:
- trunk hidden states are mapped through a small residual output projector
- token scores are negative squared distances to a learned output-center table
- the output centers are separate from the input token embeddings

Training objective:
- direct metric-CE/NLL by default
- optional TFSD loss on the projected metric logits
"""
...
class Hyperparameters(base.Hyperparameters):
    ...
    metric_center_lr = float(os.environ.get("TFSD_CENTER_LR", os.environ.get("HEAD_LR", "0.008")))
    metric_init_from_embed = bool(int(os.environ.get("TFSD_INIT_FROM_EMBED", "1")))


class ProjectedUntiedMetricHeadGPT(base.GPT):
    def __init__(self, args: Hyperparameters):
        ...
        self.metric_centers = nn.Parameter(torch.empty(args.vocab_size, args.model_dim))
        if args.metric_init_from_embed:
            with torch.no_grad():
                self.metric_centers.copy_(self.tok_emb.weight)
        else:
            nn.init.normal_(self.metric_centers, mean=0.0, std=args.tied_embed_init_std)
    ...
    def project_hidden(self, hidden: Tensor) -> Tensor:
        # Start from the current metric head and let the residual projector learn
        # a separate output manifold only if it helps.
        return hidden + self.output_proj(hidden)

    def metric_logits(self, hidden: Tensor) -> Tensor:
        projected = self.project_hidden(hidden)
        flat_hidden = projected.reshape(-1, projected.size(-1)).float() * self.metric_input_scale
        centers = self.metric_centers.float() * self.metric_input_scale
...
def main() -> None:
    ...
    base_model = ProjectedUntiedMetricHeadGPT(args).to(device).bfloat16()
    ...
    matrix_params = [
        ...
        if name not in {"tok_emb.weight", "lm_head.weight", "metric_centers"}
        and p.ndim == 2
    ...
    optimizer_centers = torch.optim.Adam(
        [{"params": [base_model.metric_centers], "lr": args.metric_center_lr, "base_lr": args.metric_center_lr}],
        betas=(args.beta1, args.beta2),
        eps=args.adam_eps,
        fused=True,
    )
    ...
    optimizers: list[torch.optim.Optimizer] = [optimizer_tok, optimizer_centers, optimizer_muon, optimizer_scalar]
    ...
    log0(
        f"metric_head_projected:tfsd_weight={args.tfsd_weight} metric_ce_weight={args.metric_ce_weight} "
        f"teacher_temp={args.tfsd_teacher_temp} student_temp={args.tfsd_student_temp} "
        f"metric_gamma={base_model.metric_gamma:.4f} center_lr={args.metric_center_lr}"
    )
    log0(
