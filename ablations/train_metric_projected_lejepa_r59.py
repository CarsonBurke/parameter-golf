"""
LeJEPA r59: paper-faithful SIGReg config + MSE attract, no detach.

r58 collapsed despite MSE attract in same Euclidean space as SIGReg.
Diagnosis check: our SIGReg config diverged from the paper:
  - num_slices: 64 (paper recommends 1024)
  - t-grid: [0, 4] one-sided (paper uses [-5, 5] symmetric)

The truncated t-range is the bigger concern. Low-rank distributions
(empirical CF ≈ 1) deviate most from the Gaussian CF (≈0) at large |t|.
Cutting at t=4 truncates exactly where the collapse signal lives.

r59 recipe — same as r58 but with paper-faithful SIGReg:
  num_slices = 1024
  t          = linspace(-5, 5, 17)
  weighting  = exp(-t²/2)  (gaussian CF)

Loss:
  attract = mean((hidden − tgt)²)          # NO detach
  loss    = attract + 0.05 · SIGReg(hidden) + 0.05 · SIGReg(tok_emb.weight)

If r59 still collapses, this confirms a structural mismatch between
LeJEPA's setup (two encoder views with architectural diversity) and ours
(transformer hidden + lookup-table codebook). The codebook has no
input-conditional structure to resist attract's pull, so SIGReg's
marginal constraint is fundamentally insufficient.

Hyperparameters (env):
  LEJEPA_SIGREG_WEIGHT_HIDDEN  — λ_h, default 0.05
  LEJEPA_SIGREG_WEIGHT_CB      — λ_c, default 0.05
  LEJEPA_SIGREG_PROJECTIONS    — 1024 (paper default)
  LEJEPA_SIGREG_NUM_POINTS     — 17
  LEJEPA_SIGREG_T_MAX          — 5.0 (range becomes [-t_max, t_max])
...
class Hyperparameters(base.Hyperparameters):
    ...
    sigreg_t_max = float(os.environ.get("LEJEPA_SIGREG_T_MAX", "5.0"))
...
class LeJEPAMetricGramGPT(base.GPT):
    def __init__(self, args: Hyperparameters):
        ...
        t = torch.linspace(-args.sigreg_t_max, args.sigreg_t_max, args.sigreg_num_points, dtype=torch.float32)
...
def main() -> None:
    ...
    log0(
        ...
        f"head=lejepa_mse_attract_plus_paper_faithful_sigreg_nodetach"
    )
    log0(
