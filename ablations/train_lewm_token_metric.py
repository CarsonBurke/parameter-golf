"""
LeWM-style latent trunk with a detached continuous token decoder using the live token embedding table.

Training objective:
- trunk is shaped by latent prediction MSE + SIGReg
- detached decoder head predicts a continuous next-token point in embedding space
...
class LeWMTokenMetricGPT(base.GPT):
    training_only_prefixes = ("encoder_projector.", "predictor.", "predictor_projector.")
    ...
    def __init__(self, args: Hyperparameters):
        ...
        self.token_head = TokenMetricHead(args.model_dim, args.token_metric_expansion)
    ...
    def token_point(self, hidden: Tensor, *, detach_hidden: bool) -> Tensor:
        if detach_hidden:
            hidden = hidden.detach()
        return self.token_head(hidden)

    def token_metric_loss(self, hidden: Tensor, target_ids: Tensor) -> Tensor:
        pred = F.normalize(self.token_point(hidden, detach_hidden=True), dim=-1)
        target = F.normalize(self.tok_emb(target_ids), dim=-1)
        return (1.0 - (pred * target).sum(dim=-1)).mean()

    def token_logits(self, hidden: Tensor) -> Tensor:
        pred = F.normalize(self.token_point(hidden, detach_hidden=False), dim=-1)
    ...
    def eval_ce_loss(self, hidden: Tensor, target_ids: Tensor) -> Tensor:
        logits = self.token_logits(hidden)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            if self.token_metric_weight > 0.0:
                loss = loss + self.token_metric_weight * self.token_metric_loss(hidden, target_ids)
            return loss
...
def main() -> None:
    ...
    base_model = LeWMTokenMetricGPT(args).to(device).bfloat16()
