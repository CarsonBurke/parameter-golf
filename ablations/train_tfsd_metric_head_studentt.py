"""
Muon-baseline ablation with a Student-t style TFSD metric decoder.

Decoder:
- one tied centroid per token
- token probabilities come from a normalized heavy-tailed distance kernel
  K(d) = (1 + gamma * d)^(-beta)

Training objective:
- pure TFSD on the kernel-normalized token distribution
- no CE anchor by default
"""
...
class Hyperparameters(base.Hyperparameters):
    ...
    kernel_beta = float(os.environ.get("TFSD_KERNEL_BETA", "1.0"))


class StudentTMetricHeadGPT(base.GPT):
    def __init__(self, args: Hyperparameters):
        ...
        self.kernel_beta = args.kernel_beta
    ...
    def log_kernel(self, dist: Tensor, temperature: float = 1.0) -> Tensor:
        ...
        return -(self.kernel_beta / safe_temp) * torch.log1p(self.metric_gamma * dist.clamp_min(0.0))
...
def main() -> None:
    ...
    base_model = StudentTMetricHeadGPT(args).to(device).bfloat16()
    ...
    log0(
        f"studentt_metric:tfsd_weight={args.tfsd_weight} metric_ce_weight={args.metric_ce_weight} "
        f"teacher_temp={args.tfsd_teacher_temp} student_temp={args.tfsd_student_temp} "
        f"metric_gamma={args.metric_gamma} kernel_beta={args.kernel_beta}"
    )
    log0(
