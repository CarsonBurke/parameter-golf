"""
LeJEPA r46: direct full-token-geometry Huber loss + SIGReg.
...
r46 keeps the same "predict one continuous embed z" trunk as r44, but stops
...
geometry induced by z against the full token geometry of the true target row:
  ...
  loss        = (1-λ)·geom + λ·sigreg_tgt + η·sigreg_pred
...
Diagnostics: geom, geom_mae, pos_cos, sigreg_tgt, sigreg_pred, sigreg_h
...
class LeJEPAGeometryGPT(base.GPT):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        self.last_ce.copy_(geom.detach())
        ...
        if self.use_additive:
            return geom + self.sigreg_weight * sigreg_tgt + self.pred_sigreg_weight * sigreg_pred
        return (1.0 - self.lejepa_lambda) * geom + self.lejepa_lambda * sigreg_tgt + self.pred_sigreg_weight * sigreg_pred
...
def main() -> None:
    ...
    base_model = LeJEPAGeometryGPT(args).to(device).bfloat16()
    ...
    log0(
        ...
        f"head=full_token_geometry_huber"
    )
    log0(
